...
// (We will leave mockDesigns here temporarily so your 3rd tab doesn't look empty!)
const mockDesigns = [
...
export default function AccountLogin() {
  ...
  useEffect(() => {
    const fetchMyData = async () => {
      ...
      try {
        ...
        const ordersRes = await fetch('http://127.0.0.1:8000/api/orders/history/', { headers });
        
        if (ordersRes.ok) {
          const ordersData = await ordersRes.json();

          // 🚨 THE FIX: Translate Django's variable names into React's expected names
          const formattedOrders = ordersData.map(order => {
            
            // Safely grab the first item from Django's items array (if it exists)
            const firstItem = order.items && order.items.length > 0 ? order.items[0] : null;

            return {
              ...order,
              // Use the cleaner order_code if available, otherwise fallback to database ID
              id: order.order_code || order.id,
              
              // Format the ugly database timestamp into a clean string (e.g., "April 19, 2026")
              date: new Date(order.date).toLocaleDateString('en-US', { 
                month: 'long', day: 'numeric', year: 'numeric' 
              }),
              
              // Translate total_price to total
              total: parseFloat(order.total_price) || 0,
              
              // Extract the service name and quantity from the first item in the cart
              service: firstItem ? firstItem.service_name : "Custom Order",
              qty: firstItem ? firstItem.quantity : 0,
            };
          });

          // Save our newly formatted data to React state!
  ...
  // Filter the live orders!
  const filteredOrders = orderFilter === 'All' ? orders : orders.filter(order => order.status && order.status.toLowerCase() === orderFilter.toLowerCase());

  const handleEditProfile = () => alert("[Backend Placeholder] Save profile API goes here");
  ...
  const StatusBadge = ({ status }) => {
    ...
    return (
      <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${colors[normalizedStatus] || "bg-gray-50 text-gray-600"}`}>
        {status || "Pending"}
      </span>
    );
  };
    ...
    <div className="flex-1 bg-gray-300 py-16 font-sans min-h-screen">
      <div className="max-w-[90rem] mx-auto px-6 md:px-12 flex flex-col lg:flex-row gap-12">
        ...
        <div className="w-full lg:w-1/3 xl:w-1/4">
          <div className="bg-white rounded-[2rem] shadow-xl border border-gray-200 p-10 h-fit transition-all duration-500">
            ...
            <div className="flex flex-col items-center text-center gap-6 mb-10 pb-10 border-b border-gray-100">
              ...
              <div>
                {/* LIVE DATA: full_name */}
                <p className="text-2xl font-thin tracking-widest text-gray-900 uppercase">{profile.full_name}</p>
                <button 
            ...
            <nav className="space-y-3">
              ...
              <div className="pt-8 mt-6 border-t border-gray-100">
                {/* Optional: You can add an onClick here to clear localStorage to actually log them out! */}
        ...
        <div className="w-full lg:w-2/3 xl:w-3/4">
              ...
              <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); alert('Profile Saved!'); }}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-3">
                    ...
                    <input type="text" value={profile.full_name || ""} onChange={(e) => setProfile({...profile, full_name: e.target.value})} className="w-full bg-gray-50 border-b-2 border-gray-200 focus:border-red-600 rounded-2xl p-5 text-gray-900 font-light transition-all outline-none text-lg" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] ml-1">Phone Number</label>
                    <input type="tel" value={profile.phone || ""} onChange={(e) => setProfile({...profile, phone: e.target.value})} className="w-full bg-gray-50 border-b-2 border-gray-200 focus:border-red-600 rounded-2xl p-5 text-gray-900 font-light transition-all outline-none text-lg" />
                  </div>
                  <div className="md:col-span-2 space-y-3">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] ml-1">Email Address</label>
                    <input type="email" value={profile.email || ""} onChange={(e) => setProfile({...profile, email: e.target.value})} className="w-full bg-gray-50 border-b-2 border-gray-200 focus:border-red-600 rounded-2xl p-5 text-gray-900 font-light transition-all outline-none text-lg" />
                  </div>
                  <div className="md:col-span-2 space-y-3">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] ml-1">Delivery Address</label>
                    <textarea rows="4" value={profile.address || ""} onChange={(e) => setProfile({...profile, address: e.target.value})} className="w-full bg-gray-50 border-b-2 border-gray-200 focus:border-red-600 rounded-2xl p-5 text-gray-900 font-light transition-all outline-none resize-none text-lg leading-relaxed"></textarea>
                  </div>
                </div>
                <div className="pt-6">
            ...
            <div className="animate-fade-in space-y-8">
              ...
              <div className="space-y-8">
                {filteredOrders.length === 0 ? (
                  <div className="bg-white rounded-[2rem] p-10 text-center text-gray-500 font-light tracking-wide">
                    ...
                    <div key={order.id} className="bg-white rounded-[2rem] shadow-xl border border-gray-200 overflow-hidden hover:border-red-100 transition-colors duration-500">
                      ...
                      <div className="p-10 flex flex-col md:row gap-10 items-center">
                        <div className="w-32 h-32 bg-gray-100 rounded-[2rem] overflow-hidden border border-gray-200 shrink-0 shadow-inner">
                          <img src={order.image || "https://placehold.co/200x200?text=Product"} alt={order.service} className="w-full h-full object-cover grayscale hover:grayscale-0 transition-all duration-700" onError={(e) => {e.target.src='https://placehold.co/200x200?text=Product'}} />
                        </div>
                        ...
                        <div className="md:text-right w-full md:w-auto border-t md:border-t-0 pt-6 md:pt-0 border-gray-100">
                          ...
                          <p className="text-4xl font-thin text-red-600">₱{order.total ? order.total.toLocaleString() : 0}</p>
                        </div>
                      </div>
                    </div>
                  ))
          ...
          {/* TAB 3: SAVED DESIGNS (Still Mock Data for now) */}
          {activeTab === 'designs' && (
            <div className="bg-white rounded-[2rem] shadow-xl border border-gray-200 p-10 md:p-14 animate-fade-in">
