  ...
  return (
    <button
      ...
      className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-left w-full transition-all ${
        active ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-600 hover:border-red-300"
      }`}
    >
          ...
          >
            ...
            <div className="absolute inset-2 flex flex-col items-center justify-center gap-1 text-center">
              {designMode === "design" && businessName ? (
                <>
                  <p className="text-[9px] font-black text-gray-800 leading-tight break-words w-full">{businessName}</p>
                  {tagline && <p className="text-[7px] text-gray-500 leading-tight break-words w-full">{tagline}</p>}
                  <div className="w-8 border-t border-red-300 my-0.5" />
                  <div className="w-full flex flex-col gap-0.5 px-1">
                    {[70, 50, 60, 45].map((pct, i) => (
                      <div key={i} className="h-[3px] bg-gray-200 rounded-full" style={{ width: `${pct}%`, margin: "0 auto" }} />
                    ))}
                  </div>
                </>
              ) : (
            ...
            >
              <div className="flex flex-col items-center gap-0.5 opacity-40">
                {[55, 70, 45, 60, 50].map((pct, i) => (
                  <div key={i} className="h-[2px] bg-gray-300 rounded-full" style={{ width: `${pct}%` }} />
                ))}
              </div>
            </div>
...
export default function FlyersOrderForm() {
  ...
  const PAPER_TYPES = [
    { val: "Glossy Paper", icon: <FaImage />, desc: "Shiny, vibrant colors" },
    { val: "Matte Paper", icon: <FaRegFileLines />, desc: "Smooth, no-glare finish" },
    { val: "Cardstock", icon: <FaBox />, desc: "Thick, premium feel" },
    { val: "Recycled Paper", icon: <FaRecycle />, desc: "Eco-friendly option" },
  ]
  ...
  const FOLDING_OPTS = [
    { val: "None", icon: <FaRegFileLines />, label: "None" },
    { val: "Bi-fold", icon: <FaLayerGroup />, label: "Bi-fold" },
    { val: "Tri-fold", icon: <FaLayerGroup />, label: "Tri-fold" },
  ]
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 flex flex-col gap-6">
        <SectionCard title="Flyer Details" icon={<FaRegFileLines />}>
            ...
            <Field label="Orientation">
              <div className="flex gap-3">
                <ToggleBtn active={orientation === "Portrait"} onClick={() => setOrientation("Portrait")}>
                  <FaMobileScreenButton className="inline-block mr-2 align-[-0.1em]" /> Portrait
                </ToggleBtn>
                <ToggleBtn active={orientation === "Landscape"} onClick={() => setOrientation("Landscape")}>
                  <FaDesktop className="inline-block mr-2 align-[-0.1em]" /> Landscape
                </ToggleBtn>
              </div>
            </Field>
        ...
        <SectionCard title="Paper & Finish" icon={<FaPalette />}>
          <div className="flex flex-col gap-5">
            <Field label="Paper Type">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {PAPER_TYPES.map(({ val, icon, desc }) => (
                  <OptionCard key={val} active={paperType === val} onClick={() => setPaperType(val)} icon={icon} label={val} sublabel={desc} />
                ))}
              </div>
            </Field>
            ...
            <Field label="Surface Finish">
              <div className="flex flex-col gap-2">
                {FINISHES.map(({ val, desc, badge }) => (
                  <OptionCard key={val} active={finish === val} onClick={() => setFinish(val)} label={val} sublabel={desc} badge={badge} />
                ))}
              </div>
            </Field>
          </div>
        ...
        <SectionCard title="Design" icon={<FaPalette />}>
          <div className="flex flex-col gap-5">
            ...
            <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
              ...
              >
                <FaFolderOpen className="inline-block mr-2 align-[-0.1em]" /> Upload Design File
              </button>
              <button
                type="button"
                onClick={() => setDesignMode("design")}
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${
                  designMode === "design" ? "bg-white text-gray-800 shadow-sm" : "text-gray-500 hover:text-gray-700"
                }`}
              >
                <FaPenToSquare className="inline-block mr-2 align-[-0.1em]" /> Request Design Assistance
              </button>
            </div>
            ...
            {designMode === "upload" ? (
              <Field label="Upload Your Artwork" hint="PNG, JPG, PDF, AI accepted · 300 DPI recommended · Max 100MB" required>
                ...
                >
                  <span className="text-4xl group-hover:scale-110 transition-transform">
                    {designFile ? <FaCircleCheck /> : <FaImage />}
                  </span>
                  {designFile ? (
                ...
                <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-1">
                  <FaCircleInfo />
                  <p>
                    For best results, supply files at <strong>300 DPI</strong> in CMYK color mode with <strong>3mm bleed</strong> on all sides.
                  </p>
                </div>
              </Field>
            ) : (
              <div className="flex flex-col gap-4">
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-4 text-sm text-amber-700">
                  <FaWandMagicSparkles />
                  <p>
                    Our design team will create your flyer based on the details below. <strong>Additional design fees may apply.</strong> A proof will be
                    sent for approval before printing.
                  </p>
                </div>
                ...
                <Field label="Description / Main Content" hint="Key details to include on the flyer">
                  <textarea
                    ...
                    rows={3}
                    placeholder="e.g. 50% off all drinks this Saturday, June 14 · Live music from 6 PM"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Preferred Color Scheme">
                    <input
                      ...
                      placeholder="e.g. Red & black, warm earthy tones…"
                      className={inputCls}
                    />
                  </Field>
                </div>
        ...
        <SectionCard title="Versions" icon={<FaClipboardList />}>
          <div className="flex flex-col gap-4">
            <div className="flex gap-3">
              <ToggleBtn active={!multiVersion} onClick={() => setMultiVersion(false)}>
                <FaRegFileLines className="inline-block mr-2 align-[-0.1em]" /> Single Version
              </ToggleBtn>
              <ToggleBtn active={multiVersion} onClick={() => setMultiVersion(true)}>
                <FaFileCsv className="inline-block mr-2 align-[-0.1em]" /> Multiple Versions (CSV)
              </ToggleBtn>
            </div>
            {multiVersion ? (
              <div className="flex flex-col gap-3">
                <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
                  <FaClipboardList className="text-lg shrink-0 mt-0.5" />
                  <div>
                    <p className="font-bold mb-1">Multiple Versions — CSV Upload</p>
                    <p className="text-xs leading-relaxed">
                      Upload a file with columns: <code className="bg-blue-100 px-1 rounded">Name</code>,{" "}
                      <code className="bg-blue-100 px-1 rounded">Branch</code>,{" "}
                      <code className="bg-blue-100 px-1 rounded">Content</code>. Each row = one unique flyer version.
                    </p>
                  </div>
                </div>
                <Field label="Upload CSV / Excel File" required>
                  ...
                  >
                    <span className="text-3xl group-hover:scale-110 transition-transform">{csvFile ? <FaCircleCheck /> : <FaFileCsv />}</span>
                    {csvFile ? (
              ...
              <div className="flex items-start gap-3 bg-green-50 border border-green-100 rounded-xl p-4 text-sm text-green-700">
                <FaRegFileLines />
                <p>
                  <strong>Single version</strong> — all {qty.toLocaleString()} flyer{qty !== 1 ? "s" : ""} will be printed with the same design.
                </p>
              </div>
            )}
          </div>
        ...
        <SectionCard title="Add-ons & Finishing" icon={<FaLayerGroup />}>
          <div className="flex flex-col gap-5">
                ...
                {FOLDING_OPTS.map(({ val, icon, label }) => (
                  <button
                    ...
                    className={`flex flex-col items-center gap-1 py-3 px-2 rounded-xl border-2 text-xs font-bold text-center transition-all ${
                      folding === val ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-600 hover:border-red-300"
                    }`}
                  >
                    <span className="text-xl">{icon}</span>
                    {label}
                    {val !== "None" && (
                      <span className={`text-[9px] font-black ${folding === val ? "text-red-400" : "text-gray-400"}`}>+₱2</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <AddOnToggle
                label="UV Coating"
                sublabel="High-gloss spot UV for premium look"
                icon={<FaShieldHalved />}
                price={2}
                checked={uvCoating}
              ...
              <AddOnToggle
                label="Lamination"
                sublabel="Protective film over entire surface"
                icon={<FaShieldHalved />}
                price={3}
                checked={lamination}
                onChange={() => setLamination(!lamination)}
              />
              <AddOnToggle
                label="Rounded Corners"
                sublabel="Smooth 4mm radius on all corners"
                icon={<FaCropSimple />}
                price={1}
              ...
              <AddOnToggle
                ...
                sublabel="Priority — 24–48 hr turnaround"
                icon={<FaBolt />}
                price={3}
                checked={express}
        ...
        <SectionCard title="Delivery" icon={<FaTruck />}>
          <div className="flex flex-col gap-4">
        ...
        <div className="sticky top-[8.75rem] flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="px-6 pb-6 flex flex-col gap-3">
              ...
              <p className="text-[11px] text-gray-400 text-center">We'll confirm and send a payment link within 24 hours.</p>
            </div>
          </div>
