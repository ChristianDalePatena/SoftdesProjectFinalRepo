  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
            ...
            <>
              ...
              <div className="flex flex-col gap-0.5 w-full items-center mt-1">
                {[70, 50, 60, 45, 55, 40].map((pct, i) => (
                  <div key={i} className="h-0.5 rounded-full bg-gray-300" style={{ width: `${pct}%` }} />
                ))}
              </div>
      ...
      {includeStand && !printOnly && (
        <svg width={svgW} height={svgH} viewBox={`0 0 ${svgW} ${svgH}`} className="overflow-visible" style={{ marginTop: -2 }}>
          <line
            x1={svgW / 2 - armSpread}
            y1={2}
            x2={svgW / 2 + armSpread}
            y2={svgH - 4}
            stroke={standColor}
            strokeWidth={standType === "Heavy Duty X-Stand" ? 2.5 : 1.8}
            strokeLinecap="round"
          />
          <line
            x1={svgW / 2 + armSpread}
      ...
      {includeStand && !printOnly && <p className="text-[9px] text-gray-500 font-semibold">{standType}</p>}
      {printOnly && <p className="text-[9px] text-amber-600 font-semibold">Print Only</p>}
    </div>
  )
}
...
export default function XBannerOrderForm() {
  ...
  const MATERIALS = [
    { val: "Tarpaulin (Matte)", icon: <TreePine className="h-4 w-4" />, desc: "Durable, no-glare outdoor material" },
    { val: "Tarpaulin (Glossy)", icon: <Sparkles className="h-4 w-4" />, desc: "Vibrant colors, weather-resistant" },
    { val: "Synthetic Paper", icon: <FileText className="h-4 w-4" />, desc: "Smooth, tear-resistant, indoor use" },
  ]
  ...
  const summaryRows = [
    ...
    { label: "Quality", value: printQuality + (printQuality === "High Resolution" ? " (+₱100)" : "") },
    {
      ...
      value: printOnly
        ? "Print Only (−30%)"
        : includeStand
          ? standType + (standType === "Heavy Duty X-Stand" ? " (+₱300)" : "")
          : "No Stand",
    },
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 flex flex-col gap-6">
        {/* Banner Details */}
        <SectionCard title="Banner Details" icon={<Flag className="h-5 w-5" />}>
        ...
        {/* Print & Material */}
        <SectionCard title="Print & Material" icon={<Printer className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            <Field label="Material Type">
              <div className="flex flex-col gap-2">
                {MATERIALS.map(({ val, icon, desc }) => (
                  <OptionCard key={val} active={material === val} onClick={() => setMaterial(val)} icon={icon} label={val} sublabel={desc} />
                ))}
              </div>
            </Field>
        ...
        <SectionCard title="Stand Options" icon={<Wrench className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            <Field label="Order Type">
              <div className="flex gap-3">
                <ToggleBtn active={!printOnly} onClick={() => handlePrintOnly(false)}>
                  <Wrench className="h-4 w-4 inline-block mr-1.5" /> With X-Stand
                </ToggleBtn>
                <ToggleBtn active={printOnly} onClick={() => handlePrintOnly(true)}>
                  <Printer className="h-4 w-4 inline-block mr-1.5" /> Print Only
              ...
              {printOnly && (
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700 mt-2">
                  ...
                  <p>
                    <strong>Print Only</strong> — replacing an existing banner print? Base price is reduced by 30%. No stand included.
                  </p>
                </div>
              )}
            </Field>
                ...
                <Field label="Include X-Stand?">
                  <div className="flex gap-3">
                    <ToggleBtn active={includeStand} onClick={() => setIncludeStand(true)}>
                      <CheckCircle2 className="h-4 w-4 inline-block mr-1.5" /> Yes, Include Stand
                    </ToggleBtn>
                    <ToggleBtn active={!includeStand} onClick={() => setIncludeStand(false)}>
                      <XCircle className="h-4 w-4 inline-block mr-1.5" /> No Stand
                    </ToggleBtn>
                  </div>
                </Field>
                ...
                {includeStand && (
                  <Field label="Stand Type">
                    ...
                    <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-2">
                      ...
                      <p>
                        Both stands are collapsible and come with a carry bag. Heavy Duty is recommended for frequent transport or outdoor events.
                      </p>
                    </div>
                  </Field>
                )}
        ...
        {/* Finishing */}
        <SectionCard title="Finishing" icon={<Star className="h-5 w-5" />}>
            ...
            >
              ...
              {!needsDesign && (
                <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-1">
                  <Lightbulb className="h-4 w-4 shrink-0 mt-0.5" />
                  <p>
                    Supply files at <strong>300 DPI</strong> in CMYK mode. Recommended canvas size matches your selected banner dimensions with{" "}
                    <strong>3mm bleed</strong> on all sides.
                  </p>
                </div>
              )}
            </Field>
