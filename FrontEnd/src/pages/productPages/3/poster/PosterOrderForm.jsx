...
function PosterPreview({ size, orientation, colorMode, lamination, usageType, hasDesign }) {
  ...
  const usageIcons = {
    "Event Poster": <MdStar className="text-base" />,
    "Movie Poster": <MdMovie className="text-base" />,
    "Educational Poster": <MdMenuBook className="text-base" />,
    Advertisement: <MdGridView className="text-base" />,
    Others: <MdImage className="text-base" />,
  }
    ...
    <div className="flex flex-col items-center gap-2">
      <div
        ...
        style={{
          ...
          background: bgColor,
          boxShadow:
            lamination !== "None" ? "3px 3px 10px rgba(0,0,0,0.15)" : "1px 1px 4px rgba(0,0,0,0.08)",
        }}
      >
        {lamination !== "None" && (
        ...
        <div className="absolute inset-2 flex flex-col items-center justify-between p-1">
          ...
          <div className="flex flex-col items-center gap-1 text-center flex-1 justify-center">
            {hasDesign ? (
              <>
                {usageIcons[usageType] || <MdImage className="text-base" />}
                <div className="flex flex-col gap-0.5 w-full px-1">
                  {[65, 45, 55, 40, 50].map((pct, i) => (
                    <div
                      ...
                      style={{
                        width: `${pct}%`,
                        background: colorMode === "Black & White" ? "#9ca3af" : "#fca5a5",
                      }}
                    />
                  ))}
                </div>
              ...
              <div className="flex flex-col items-center gap-1 opacity-30">
                <MdImage className="text-base text-gray-400" />
...
export default function PosterOrderForm() {
  ...
  const PAPER_TYPES = [
    { val: "Matte Paper", icon: <MdBrush className="text-lg shrink-0" />, desc: "Smooth, no-glare finish" },
    { val: "Glossy Paper", icon: <MdStar className="text-lg shrink-0" />, desc: "Shiny, vibrant colors" },
    { val: "Photo Paper", icon: <MdImage className="text-lg shrink-0" />, desc: "High-detail photo quality" },
    { val: "Art Paper", icon: <MdPalette className="text-lg shrink-0" />, desc: "Rich, gallery-grade texture" },
  ]

  const summaryRows = [
    {
      label: "Size",
      value: posterSize === "Custom" ? `Custom (${customW || "?"}×${customH || "?"})` : posterSize,
    },
    ...
    ...(mounting !== "None"
      ? [{ label: "Mounting", value: `${mounting} (+₱${mounting === "Foam Board" ? 50 : 100})` }]
      : []),
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 flex flex-col gap-6">
        {/* Poster Details */}
        <SectionCard title="Poster Details" icon={<MdImage className="text-xl" />}>
              ...
              <div className="flex gap-3">
                <ToggleBtn active={orientation === "Portrait"} onClick={() => setOrientation("Portrait")}>
                  <span className="inline-flex items-center gap-1">
                    <MdSmartphone className="text-sm" /> <span>Portrait</span>
                  </span>
                </ToggleBtn>
                <ToggleBtn active={orientation === "Landscape"} onClick={() => setOrientation("Landscape")}>
                  <span className="inline-flex items-center gap-1">
                    <MdDesktopWindows className="text-sm" /> <span>Landscape</span>
                  </span>
                </ToggleBtn>
              </div>
            </Field>
        ...
        {/* Paper Options */}
        <SectionCard title="Paper Options" icon={<MdDescription className="text-xl" />}>
          <div className="flex flex-col gap-5">
        ...
        {/* Printing Options */}
        <SectionCard title="Printing Options" icon={<MdPrint className="text-xl" />}>
          <div className="flex flex-col gap-5">
            <Field label="Print Quality">
              <div className="flex flex-col gap-2">
                <OptionCard
                  ...
                  icon={<MdPrint className="text-lg shrink-0" />}
                  label="Standard"
                ...
                <OptionCard
                  ...
                  icon={<MdVisibility className="text-lg shrink-0" />}
            ...
            <Field label="Color Mode">
              <div className="flex gap-3">
                <ToggleBtn active={colorMode === "Full Color"} onClick={() => setColorMode("Full Color")}>
                  <span className="inline-flex items-center gap-1">
                    <MdPalette className="text-sm" /> <span>Full Color</span>{" "}
                    <span className="text-[10px] font-black opacity-80 ml-1">+₱10</span>
                  </span>
                </ToggleBtn>
                <ToggleBtn active={colorMode === "Black & White"} onClick={() => setColorMode("Black & White")}>
                  <span className="inline-flex items-center gap-1">
                    <MdCropSquare className="text-sm" /> <span>Black & White</span>
                  </span>
                </ToggleBtn>
              </div>
            </Field>
        ...
        {/* Finishing Options */}
        <SectionCard title="Finishing Options" icon={<MdStar className="text-xl" />}>
          <div className="flex flex-col gap-5">
            <Field label="Lamination">
              <div className="flex flex-col gap-2">
                {[
                  { val: "None", icon: <MdClose className="text-lg shrink-0" />, desc: "No coating applied" },
                  { val: "Matte", icon: <MdBrush className="text-lg shrink-0" />, desc: "Smooth, fingerprint-resistant", badge: "+₱15" },
                  { val: "Glossy", icon: <MdStar className="text-lg shrink-0" />, desc: "Shiny protective film", badge: "+₱15" },
                ].map(({ val, icon, desc, badge }) => (
                  <OptionCard
                    key={val}
                    active={lamination === val}
            ...
            <Field label="Mounting">
              <div className="flex flex-col gap-2">
                {[
                  { val: "None", icon: <MdClose className="text-lg shrink-0" />, desc: "Poster only, no backing" },
                  { val: "Foam Board", icon: <MdCropSquare className="text-lg shrink-0" />, desc: "Lightweight rigid backing", badge: "+₱50" },
                  { val: "Sintra Board", icon: <MdLayers className="text-lg shrink-0" />, desc: "Durable PVC board backing", badge: "+₱100" },
                ].map(({ val, icon, desc, badge }) => (
                  <OptionCard
                    key={val}
                    active={mounting === val}
                    onClick={() => setMounting(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                    badge={badge}
            ...
            <Field label="Framing">
              <div className="flex gap-3">
                <ToggleBtn active={framing === "None"} onClick={() => setFraming("None")}>
                  <span className="inline-flex items-center gap-1">
                    <MdClose className="text-sm" /> <span>No Frame</span>
                  </span>
                </ToggleBtn>
                <ToggleBtn active={framing === "With Frame"} onClick={() => setFraming("With Frame")}>
                  <span className="inline-flex items-center gap-1">
                    <MdImage className="text-sm" /> <span>With Frame</span>{" "}
                    <span className="text-[10px] font-black opacity-80 ml-1">+₱150</span>
                  </span>
                </ToggleBtn>
              </div>
            </Field>
        ...
        {/* Design */}
        <SectionCard title="Design" icon={<MdEdit className="text-xl" />}>
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              >
                <span className="text-4xl group-hover:scale-110 transition-transform">
                  {designFile ? <MdCheck /> : <MdFileUpload />}
                </span>
                {designFile ? (
              ...
              {!needsDesign && (
                <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-1">
                  <MdInfoOutline />
                  <p>
                    For best results, supply files at <strong>300 DPI</strong> in CMYK mode with <strong>3mm bleed</strong> on all sides.
                  </p>
                </div>
              )}
            </Field>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (!needsDesign) setDesignFile(null)
              }}
              className={`flex items-center gap-4 p-4 rounded-xl border-2 text-left w-full transition-all ${
            ...
            >
              ...
              >
                {needsDesign && <MdCheck className="text-white text-[10px] font-black" />}
              </div>
            </button>
        ...
        {/* Usage Type */}
        <SectionCard title="Usage Type" icon={<MdGridView className="text-xl" />}>
          <div className="flex flex-col gap-4">
            <Field label="What is this poster for?">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {[
                  { val: "Event Poster", icon: <MdStar className="text-xl" /> },
                  { val: "Movie Poster", icon: <MdMovie className="text-xl" /> },
                  { val: "Educational Poster", icon: <MdMenuBook className="text-xl" /> },
                  { val: "Advertisement", icon: <MdGridView className="text-xl" /> },
                  { val: "Others", icon: <MdImage className="text-xl" /> },
                ].map(({ val, icon }) => (
                  <button
                    key={val}
                    type="button"
        ...
        {/* Delivery */}
        <SectionCard title="Delivery" icon={<MdLocalShipping className="text-xl" />}>
          <div className="flex flex-col gap-4">
            <Field label="Fulfillment Method">
              <div className="flex gap-3">
                <ToggleBtn active={delivery === "Pickup"} onClick={() => setDelivery("Pickup")}>
                  <span className="inline-flex items-center gap-1">
                    <MdLocationOn className="text-sm" /> <span>Pickup</span>
                  </span>
                </ToggleBtn>
                <ToggleBtn active={delivery === "Delivery"} onClick={() => setDelivery("Delivery")}>
                  <span className="inline-flex items-center gap-1">
                    <MdLocalShipping className="text-sm" /> <span>Delivery</span>
                  </span>
                </ToggleBtn>
              </div>
            </Field>
