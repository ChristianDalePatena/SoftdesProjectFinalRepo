...
function BannerPreview({ width, height, material, quality, eyelets, rope, usage }) {
  ...
  const glowStyle = material === "Backlit (for lightboxes)" ? { boxShadow: "0 0 20px rgba(37,99,235,0.5), 0 0 40px rgba(37,99,235,0.2)" } : {}

  const UsageIcon =
    {
      "Store Signage": Store,
      Billboard: Megaphone,
      "Event Banner": PartyPopper,
      "Promotional Ad": Megaphone,
    ...
    <div className="flex flex-col items-center gap-3">
      <div className="relative w-full h-40 rounded-xl border border-gray-200 bg-linear-to-br from-gray-100 to-gray-50 flex items-center justify-center overflow-hidden">
        <div
          ...
          style={{
            backgroundImage:
              "repeating-linear-gradient(0deg, #9ca3af 0px, #9ca3af 1px, transparent 1px, transparent 24px), repeating-linear-gradient(90deg, #9ca3af 0px, #9ca3af 1px, transparent 1px, transparent 24px)",
          }}
        />
        ...
        {rope && (
          <>
            <div
              className="absolute top-4 left-8 right-8 h-0.5 bg-yellow-600 opacity-60"
              style={{ top: `${(140 - clampedH) / 2 - 6}px` }}
            />
          </>
        )}
        ...
        >
          ...
          <div className="flex flex-col items-center gap-1 px-2 z-10">
            <UsageIcon className="h-5 w-5 text-white" />
...
export default function PanaFlexOrderForm() {
  ...
  const USAGE_OPTIONS = [
    { val: "Store Signage", icon: <Store className="h-4 w-4" /> },
    { val: "Billboard", icon: <Megaphone className="h-4 w-4" /> },
    { val: "Event Banner", icon: <PartyPopper className="h-4 w-4" /> },
    { val: "Promotional Ad", icon: <Megaphone className="h-4 w-4" /> },
    { val: "Others", icon: <MoreHorizontal className="h-4 w-4" /> },
  ]
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 flex flex-col gap-6">
        {/* Panaflex Details */}
        <SectionCard title="Panaflex Details" icon={<Signpost className="h-5 w-5" />}>
        ...
        {/* Live Preview */}
        <SectionCard title="Banner Preview" icon={<Eye className="h-5 w-5" />}>
          <BannerPreview width={width} height={height} material={material} quality={quality} eyelets={eyelets} rope={rope} usage={usage} />
        </SectionCard>

        {/* Material & Type */}
        <SectionCard title="Material & Type" icon={<Layers3 className="h-5 w-5" />}>
        ...
        {/* Printing Options */}
        <SectionCard title="Printing Options" icon={<Printer className="h-5 w-5" />}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <SectionCard title="Finishing & Installation" icon={<Wrench className="h-5 w-5" />}>
          <div className="flex flex-col gap-3">
            ...
            {rope && !eyelets && (
              <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800">
                ...
                <p>
                  Rope requires eyelets for attachment. Consider enabling <strong>Eyelets</strong> as well.
                </p>
              </div>
            )}
          </div>
        ...
        {/* Usage Type */}
        <SectionCard title="Usage Type" icon={<MapPin className="h-5 w-5" />}>
          <div className="flex flex-col gap-4">
            ...
            {usage === "Billboard" && (
              <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700">
                ...
                <p>
                  <strong>Billboard tip:</strong> We recommend Heavy Duty thickness + UV Print ink for long-term outdoor billboard installations.
                </p>
              </div>
            )}
            {usage === "Event Banner" && (
              <div className="flex items-start gap-3 bg-green-50 border border-green-100 rounded-xl p-3 text-xs text-green-700">
                <span>
                  <PartyPopper className="h-4 w-4 mt-0.5" />
                </span>
                <p>
                  <strong>Event tip:</strong> Eyelets + Rope makes setup easy for stage backdrops, entrance banners, and photo walls.
                </p>
              </div>
            )}
          </div>
        ...
        {/* Design */}
        <SectionCard title="Design" icon={<Palette className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
              ...
              <p>
                For large-format printing, provide files at <strong>100 DPI at full size</strong> (or 300 DPI at 1/3 scale). Use <strong>CMYK color mode</strong> and maintain a <strong>5mm bleed</strong> on all sides.
              </p>
            </div>
            ...
            <Field label="Upload Design File" hint="PNG, JPG, PDF, AI, EPS · CMYK preferred · Max file size 500MB">
              ...
              >
                <span className="text-3xl group-hover:scale-110 transition-transform">
                  {file ? <Check className="h-8 w-8 text-green-600" /> : <Upload className="h-8 w-8 text-gray-500" />}
                </span>
                {file ? (
            ...
            {file && (
              <div className="flex items-center justify-between bg-green-50 border border-green-100 rounded-xl px-4 py-3">
                ...
                <button type="button" onClick={() => setFile(null)} className="text-xs text-red-400 hover:text-red-600 font-bold ml-2 shrink-0">
                  Remove
                </button>
              </div>
            )}
            ...
            <label className="flex items-start gap-3 cursor-pointer select-none group">
              ...
              >
                {needsDesign && <Check className="h-3 w-3 text-white" />}
              </div>
        ...
        {/* Delivery */}
        <SectionCard title="Delivery Info" icon={<Truck className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            ...
            <Field label="Fulfillment Method">
              <div className="flex gap-3">
                <ToggleBtn active={delivery === "Pickup"} onClick={() => setDelivery("Pickup")}>
                  <Store className="h-4 w-4 inline-block mr-1.5" /> Pickup
                </ToggleBtn>
                <ToggleBtn active={delivery === "Delivery"} onClick={() => setDelivery("Delivery")}>
                  <Truck className="h-4 w-4 inline-block mr-1.5" /> Delivery
                </ToggleBtn>
              </div>
            </Field>
            {delivery === "Pickup" && (
        ...
        <div className="sticky top-35 flex flex-col gap-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-4 flex flex-col gap-1.5">
              ...
              {inkType === "UV Print" && (
                <div className="flex justify-between text-xs text-gray-500">
                  <span>UV Print Ink</span>
                  <span>+₱15/sq.{unit}</span>
                </div>
              )}
            ...
            <div className="px-6 pb-6">
              ...
              <p className="text-[11px] text-gray-400 text-center mt-3">Our team will confirm your order and send a payment link within 24 hours.</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="flex flex-col gap-2">
              <a href="tel:+639474631561" className="flex items-center gap-2 text-xs font-semibold text-red-500 hover:text-red-600 transition">
                <Phone className="h-4 w-4" /> 0947-463-1561
              </a>
              <a href="https://m.me/p2printing" target="_blank" rel="noreferrer" className="flex items-center gap-2 text-xs font-semibold text-red-500 hover:text-red-600 transition">
                <MessageCircle className="h-4 w-4" /> Chat on Messenger
              </a>
              <a href="mailto:picktwoprint@gmail.com" className="flex items-center gap-2 text-xs font-semibold text-red-500 hover:text-red-600 transition">
                <Mail className="h-4 w-4" /> picktwoprint@gmail.com
              </a>
            </div>
          </div>
        </div>
