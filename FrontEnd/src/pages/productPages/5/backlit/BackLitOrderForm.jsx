...
function BacklitPreview({ width, height, glow, material, includeFrame, hasDesign, usageType }) {
  ...
  const UsageIcon =
    {
      "Store Signage": Store,
      "Mall Advertisement": Building2,
      "Lightbox Display": Lightbulb,
      "Event Booth": PartyPopper,
      ...
      >
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center gap-1 px-2 w-full text-center">
            {hasDesign ? (
              <>
                <UsageIcon className="h-4 w-4 text-amber-700" />
                <div className="flex flex-col gap-0.5 w-full items-center">
                  {[70, 45, 60, 40, 55].map((pct, i) => (
                    <div
                      key={i}
                      className="h-0.5 rounded-full bg-amber-400 opacity-60"
                      style={{ width: `${pct}%` }}
                    />
                  ))}
                </div>
...
function computePrice({ width, height, qty, material, thickness, printQuality, installation, includeFrame }) {
  ...
  const printTotal = ratePerSqFt * area * Math.max(1, qty)
  let addons = 0
...
export default function BacklitOrderForm() {
  ...
  const summaryRows = [
    ...
    ...(installation !== "None"
      ? [{ label: "Installation", value: installation === "Basic Installation" ? "+₱200" : "+₱500" }]
      : []),
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      ...
      <div className="xl:col-span-2 flex flex-col gap-6">
        ...
        <SectionCard title="Backlit Print Details" icon={<Lightbulb className="h-5 w-5" />}>
        ...
        {/* Material Options */}
        <SectionCard title="Material Options" icon={<Layers3 className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            ...
            <Field label="Thickness">
              <div className="flex gap-3">
                <ToggleBtn active={thickness === "Standard"} onClick={() => setThickness("Standard")}>
                  <FileText className="h-4 w-4 inline-block mr-1.5" /> Standard
                </ToggleBtn>
                <ToggleBtn active={thickness === "Heavy Duty"} onClick={() => setThickness("Heavy Duty")}>
                  <Cog className="h-4 w-4 inline-block mr-1.5" /> Heavy Duty{" "}
                  <span className="text-[10px] font-black opacity-75 ml-1">+₱10/sqft</span>
                </ToggleBtn>
              </div>
        ...
        <SectionCard title="Lighting & Print Quality" icon={<SunMedium className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            ...
            <Field label="Light Transmission Level" hint="Controls how brightly the panel glows when backlit">
              <div className="flex flex-col gap-2">
                {[
                  { val: "Low Glow", icon: <Moon className="h-4 w-4" />, desc: "Subtle ambient glow — soft indoor displays" },
                  { val: "Medium Glow", icon: <Lightbulb className="h-4 w-4" />, desc: "Balanced brightness — standard lightboxes" },
                  { val: "High Brightness", icon: <SunMedium className="h-4 w-4" />, desc: "Maximum luminance — outdoor & mall signage" },
                ].map(({ val, icon, desc }) => (
                  <OptionCard key={val} active={glow === val} onClick={() => setGlow(val)} icon={icon} label={val} sublabel={desc} />
                ))}
              </div>
            </Field>
          </div>
        ...
        {/* Frame & Installation */}
        <SectionCard title="Frame & Installation" icon={<Wrench className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            <AddOnToggle
              label="Include Frame"
            ...
            <Field label="Installation Service">
              <div className="flex flex-col gap-2">
                {[
                  { val: "None", icon: <Ban className="h-4 w-4" />, desc: "No installation — pickup or delivery only" },
                  { val: "Basic Installation", icon: <Wrench className="h-4 w-4" />, desc: "Our technician handles basic mounting", badge: "+₱200" },
                  { val: "Full Installation", icon: <Cog className="h-4 w-4" />, desc: "Complete setup including wiring & framing", badge: "+₱500" },
                ].map(({ val, icon, desc, badge }) => (
                  <OptionCard key={val} active={installation === val} onClick={() => setInstallation(val)} icon={icon} label={val} sublabel={desc} badge={badge} />
                ))}
              </div>
              {installation !== "None" && (
          ...
          <div className="flex flex-col gap-4">
            <Field label="What is this backlit print for?">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {[
                  { val: "Store Signage", icon: <Store className="h-5 w-5" /> },
                  { val: "Mall Advertisement", icon: <Building2 className="h-5 w-5" /> },
                  { val: "Lightbox Display", icon: <Lightbulb className="h-5 w-5" /> },
                  { val: "Event Booth", icon: <PartyPopper className="h-5 w-5" /> },
                  { val: "Others", icon: <MoreHorizontal className="h-5 w-5" /> },
                ].map(({ val, icon }) => (
                  <button
                    key={val}
                    type="button"
        ...
        <SectionCard title="Design" icon={<Palette className="h-5 w-5" />}>
          <div className="flex flex-col gap-5">
            ...
            <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-4 text-sm text-amber-700">
              <AlertTriangle className="text-lg shrink-0 h-5 w-5" />
              <p className="text-xs leading-relaxed">
                <strong>Important for backlit printing:</strong> Use high-resolution images (300 DPI minimum) and design in{" "}
                <strong>RGB color mode</strong> for accurate light-through color reproduction. Avoid very dark backgrounds as they block light transmission.
              </p>
            </div>
            ...
            >
              ...
              >
                <span className="text-4xl group-hover:scale-110 transition-transform">
                  {designFile ? <Check className="h-9 w-9 text-green-600" /> : <Image className="h-9 w-9 text-gray-500" />}
                </span>
                {designFile ? (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-bold ${needsDesign ? "text-red-700" : "text-gray-700"}`}>I need design assistance</p>
        ...
        {/* Delivery */}
        <SectionCard title="Delivery" icon={<Truck className="h-5 w-5" />}>
          <div className="flex flex-col gap-4">
            <Field label="Fulfillment Method">
              <div className="flex gap-3">
                ...
                <ToggleBtn active={delivery === "Delivery"} onClick={() => setDelivery("Delivery")}>
                  <Package className="h-4 w-4 inline-block mr-1.5" /> Delivery
                </ToggleBtn>
              </div>
            </Field>
            {delivery === "Pickup" && (
