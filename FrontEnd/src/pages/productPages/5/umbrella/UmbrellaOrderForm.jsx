...
function OptionCard({ active, onClick, icon: Icon, label, sublabel, badge }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function UmbrellaPreview({ type, color, printCoverage, hasDesign, uvProtection }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      <svg width={cw + 20} height={ch + handleH + 20} viewBox={`0 0 ${cw + 20} ${ch + handleH + 20}`} xmlns="http://www.w3.org/2000/svg">
        <ellipse
          cx={(cw + 20) / 2}
          cy={ch + 4}
          rx={cw / 2}
        ...
        {printCoverage === "Logo Only" && <circle cx={(cw + 20) / 2} cy={ch / 2 + 4} r={Math.round(cw * 0.08)} fill="white" opacity={0.5} />}
        ...
        {uvProtection && (
          <g>
            <rect x={(cw + 20) / 2 + cw / 2 - 22} y={ch - 2} width={22} height={11} rx={3} fill="#fbbf24" opacity={0.9} />
            <text x={(cw + 20) / 2 + cw / 2 - 11} y={ch + 7} textAnchor="middle" fontSize={7} fontWeight="bold" fill="#78350f">
              UV+
            </text>
          </g>
        )}
...
export default function UmbrellaOrderForm() {
  ...
  const UMBRELLA_TYPES = [
    { val: "Folding Umbrella", icon: Umbrella, desc: "Compact & portable — fits any bag" },
    { val: "Stick Umbrella", icon: Umbrella, desc: "Classic full-size — everyday use" },
    { val: "Golf Umbrella", icon: Umbrella, desc: "Extra-large — maximum coverage" },
  ]
  ...
  const summaryRows = [
    ...
    { label: "Frame", value: frameType },
    {
      ...
      value: printCoverage + (printCoverage === "Full Canopy Print" ? " (+₱200)" : printCoverage === "Partial Print" ? " (+₱100)" : ""),
    },
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 flex flex-col gap-6">
        <SectionCard title="Umbrella Details" icon={Umbrella}>
            ...
            <Field label="Umbrella Type" required>
              <div className="flex flex-col gap-2">
                {UMBRELLA_TYPES.map(({ val, icon, desc }) => (
                  <OptionCard
                    key={val}
                    active={umbrellaType === val}
                    onClick={() => setUmbrellaType(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                ...
                {SIZES.map(({ val, desc }) => (
                  <button
                    ...
                    className={`py-2 px-3 rounded-xl border-2 text-center transition-all ${
                      size === val ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-500 hover:border-red-300"
                    }`}
                  >
        ...
        <SectionCard title="Material Options" icon={Layers3}>
          <div className="flex flex-col gap-5">
        ...
        <SectionCard title="Print Options" icon={FileText}>
          <div className="flex flex-col gap-5">
            ...
            <Field label="Print Method">
              ...
              <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-2">
                ...
                <p>
                  Sublimation is recommended for <strong>Full Canopy Print</strong> to achieve the richest, most vibrant colors.
                </p>
              </div>
            </Field>
          </div>
        ...
        <SectionCard title="Color Options" icon={Sparkles}>
                ...
                {COLORS.map((c) => (
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2 rounded-xl border-2 text-sm font-bold transition-all ${
                      baseColor === c ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-600 hover:border-red-300"
                    }`}
                  >
                    <span className="w-4 h-4 rounded-full border border-gray-300 shrink-0" style={{ background: colorDots[c] }} />
                    {c}
                  </button>
                ))}
              </div>
        ...
        <SectionCard title="Additional Features" icon={Sparkles}>
        ...
        <SectionCard title="Design" icon={Paintbrush}>
          <div className="flex flex-col gap-5">
            ...
            >
              ...
              >
                {designFile ? (
                  <CheckCircle2 className="h-10 w-10 text-green-600 group-hover:scale-110 transition-transform" />
                ) : (
                  <ImageUp className="h-10 w-10 text-gray-400 group-hover:scale-110 transition-transform" />
                )}
                {designFile ? (
              ...
              {!needsDesign && (
                <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-1">
                  <Info className="h-4 w-4 shrink-0 mt-0.5" />
                  <p>
                    Supply files at <strong>300 DPI</strong>. Curved surfaces require <strong>wrap-adjusted artwork</strong> — our team can advise on bleed setup.
                  </p>
                </div>
              )}
            </Field>
