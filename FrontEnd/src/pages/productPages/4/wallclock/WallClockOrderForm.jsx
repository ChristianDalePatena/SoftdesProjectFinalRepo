...
function OptionCard({ active, onClick, icon: Icon, label, sublabel, badge, disabled }) {
  return (
    <button
      type="button"
...
function AddOnToggle({ label, sublabel, icon: Icon, priceLabel, checked, onChange }) {
  return (
    <button
      type="button"
      onClick={onChange}
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width={130} height={160} viewBox="0 0 130 160" xmlns="http://www.w3.org/2000/svg">
        ...
        {isSquare ? (
          <rect x={17} y={27} width={96} height={96} rx={8} fill={tc.rim} stroke={tc.accent} strokeWidth={2} />
        ) : isRect ? (
          <rect x={17} y={32} width={96} height={84} rx={8} fill={tc.rim} stroke={tc.accent} strokeWidth={2} />
        ) : (
        ...
        {clockType === "Photo Wall Clock" && printStyle === "Full Face Print" && (
          <>
            ...
            <text x={cx} y={cy - 6} textAnchor="middle" fontSize={7} fill={tc.accent} fontWeight="bold">
              PHOTO
            </text>
            <text x={cx} y={cy + 4} textAnchor="middle" fontSize={6} fill={tc.accent} opacity={0.7}>
              PRINT
            </text>
          </>
        )}
        ...
        <line x1={cx} y1={cy} x2={hourX} y2={hourY} stroke={handColor} strokeWidth={3} strokeLinecap="round" />
        <line x1={cx} y1={cy} x2={minuteX} y2={minuteY} stroke={handColor} strokeWidth={2} strokeLinecap="round" />
        <line
          x1={cx}
          y1={cy}
        ...
        <text x={100} y={21} textAnchor="middle" fontSize={7.5} fontWeight="bold" fill="white">
          {sizeLabel}
        </text>

        {glowNumbers && <circle cx={cx} cy={cy} r={r} fill="none" stroke="#4ade80" strokeWidth={1.5} opacity={0.4} />}
      </svg>
...
export default function WallClockOrderForm() {
  ...
  const validate = () => {
    ...
    if (size === "Custom size" && !customSize.trim()) e.customSize = "Please enter your custom size"
    if (designMode === "upload" && !designFile && !photoFile) e.design = "Please upload a design or photo file"
  ...
  const CLOCK_TYPES = [
    { val: "Classic Analog Clock", icon: Clock3 },
    { val: "Modern Minimalist Clock", icon: Layers3 },
    { val: "Photo Wall Clock", icon: ImageUp },
    { val: "Acrylic Wall Clock", icon: Sparkles },
    { val: "Wooden Wall Clock", icon: TreePine },
  ]

  const SHAPES = [
    { val: "Round", icon: Circle },
    { val: "Square", icon: Square },
    { val: "Rectangle", icon: RectangleHorizontal },
    { val: "Custom shape", icon: PenLine },
  ]
  ...
  const MATERIALS = [
    { val: "Plastic", icon: Box, desc: "Lightweight and affordable" },
    { val: "Acrylic", icon: Sparkles, desc: "Clear, glossy modern look", badge: "+₱50" },
    { val: "Wood", icon: TreePine, desc: "Warm natural aesthetic", badge: "+₱60" },
    { val: "Glass", icon: Gem, desc: "Premium, sleek finish", badge: "+₱70" },
  ]

  const FRAMES = ["Frameless", "Plastic Frame", "Metal Frame", "Wooden Frame"]

  const MECHANISMS = [
    { val: "Standard Movement", icon: Clock3, desc: "Reliable standard ticking" },
    { val: "Silent Sweep Movement", icon: ShieldCheck, desc: "No tick — smooth silent sweep" },
  ]

  const PRINT_STYLES = [
    { val: "Full Face Print", icon: ImageUp, desc: "Design covers entire clock face" },
    { val: "Partial Design (center only)", icon: Target, desc: "Design in center, edges clean" },
    { val: "Transparent Background", icon: Sparkles, desc: "Best for acrylic or glass material" },
  ]
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-5">
            ...
            <Field label="Shape" required>
              <div className="grid grid-cols-2 gap-2">
                {SHAPES.map(({ val, icon: Icon }) => (
                  <button
                    key={val}
                    type="button"
                    onClick={() => setShape(val)}
                    className={`flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl border-2 text-sm font-bold text-center transition-all ${
                      shape === val
                        ? "border-red-500 bg-red-50 text-red-700"
                    ...
                    {s === "Large (12 inches)" && (
                      <span
                        className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                          size === s ? "bg-red-100 text-red-600" : "bg-gray-100 text-gray-500"
                        }`}
                      >
                        +₱30
                      </span>
                    )}
                    {s === "Extra Large (14+ inches)" && (
                      <span
                        className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
                          size === s ? "bg-red-100 text-red-600" : "bg-gray-100 text-gray-500"
                        }`}
        ...
        <SectionCard title="Material & Build" icon={Layers3}>
          <div className="flex flex-col gap-5">
            ...
            <Field label="Clock Mechanism" required>
              <div className="flex flex-col gap-2">
                {MECHANISMS.map(({ val, icon, desc }) => (
                  <OptionCard
                    key={val}
                    active={mechanism === val}
                    onClick={() => setMechanism(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                  />
        ...
        <SectionCard title="Customization Options" icon={Palette}>
          <div className="flex flex-col gap-5">
            <Field label="Print Style" required>
              <div className="flex flex-col gap-2">
                {PRINT_STYLES.map(({ val, icon, desc }) => (
                  <OptionCard
                    key={val}
                    active={printStyle === val}
                    onClick={() => setPrintStyle(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                  />
              ...
              {printStyle === "Transparent Background" && material !== "Acrylic" && material !== "Glass" && (
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700 mt-1">
                  <span>⚠️</span>
                  <p>
                    Transparent background works best with <strong>Acrylic</strong> or <strong>Glass</strong> material.
                    Consider changing your material selection.
                  </p>
                </div>
              )}
            </Field>
            ...
            {clockType === "Photo Wall Clock" && (
              <Field label="Photo Upload" hint="Best results with high-resolution photos" required>
                ...
                >
                  {photoFile ? (
                    <ImageUp className="h-9 w-9 group-hover:scale-110 transition-transform" />
                  ) : (
                    <ImageUp className="h-9 w-9 text-gray-400 group-hover:scale-110 transition-transform" />
                  )}
                  {photoFile ? (
            ...
            <Field label="Design Method" required>
              <div className="flex gap-3 mb-3">
                ...
                <ToggleBtn active={designMode === "text"} onClick={() => setDesignMode("text")}>
                  <Type className="inline-block h-4 w-4 mr-1.5" />
                  Text-Based
                </ToggleBtn>
              </div>
                ...
                <div className="flex flex-col gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  <Field label="Text Color">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-10 rounded-xl border border-gray-200" style={{ backgroundColor: textColor, opacity: 0.15 }} />
                    </div>
                  </Field>
                  ...
                  <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700">
                    <span>💡</span>
                    <p>
                      Our design team will create a preview and send it to you within <strong>24–48 hours</strong> for
                      approval.
                    </p>
                  </div>
                </div>
        ...
        <SectionCard title="Order Type" icon={Package}>
          <div className="flex flex-col gap-4">
            <div className="flex gap-3">
              <ToggleBtn active={!bulkOrder} onClick={() => setBulkOrder(false)}>
                <ShoppingBag className="inline-block h-4 w-4 mr-1.5" />
                Single Design
              </ToggleBtn>
              <ToggleBtn active={bulkOrder} onClick={() => setBulkOrder(true)}>
                <FileSpreadsheet className="inline-block h-4 w-4 mr-1.5" />
                Bulk Order
              </ToggleBtn>
            </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700">
                  ...
                  <div>
                    ...
                    <p>
                      Upload a CSV or Excel file with columns: <strong>Name, Date, Message</strong>. Each row will be a
                      unique clock.
                    </p>
                  </div>
                </div>
                ...
                >
                  {csvFile ? (
                    <FileSpreadsheet className="h-9 w-9 group-hover:scale-110 transition-transform" />
                  ) : (
                    <Upload className="h-9 w-9 text-gray-400 group-hover:scale-110 transition-transform" />
                  )}
                  {csvFile ? (
        ...
        <SectionCard title="Add-ons & Upgrades" icon={Sparkles}>
          <div className="flex flex-col gap-3">
            <AddOnToggle
              ...
              icon={ShieldCheck}
              priceLabel="+₱40"
              checked={silentMechanism}
              onChange={() => setSilentMechanism(!silentMechanism)}
            />
            <AddOnToggle
              ...
              priceLabel="+₱20"
              checked={glowNumbers}
              onChange={() => setGlowNumbers(!glowNumbers)}
            />
            <AddOnToggle
              ...
              icon={BadgeInfo}
              priceLabel="+₱25"
              checked={metallicHands}
              onChange={() => setMetallicHands(!metallicHands)}
            />
            <AddOnToggle
              ...
              onChange={() => setCustomPackaging(!customPackaging)}
            />
            <AddOnToggle
              ...
              priceLabel="+₱10"
              checked={batteryInclusion}
              onChange={() => setBatteryInclusion(!batteryInclusion)}
            />
          </div>
        </SectionCard>
            ...
            <Field label="Delivery Method" required>
              <div className="flex gap-3">
                ...
                <ToggleBtn active={delivery === "Delivery"} onClick={() => setDelivery("Delivery")}>
                  <Truck className="inline-block h-4 w-4 mr-1.5" />
                  Delivery
                </ToggleBtn>
              </div>
            </Field>
