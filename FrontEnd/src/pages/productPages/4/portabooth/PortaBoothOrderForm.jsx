      ...
      <div className="flex gap-2 shrink-0">
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-xs font-black border transition-all ${
            value === true ? "bg-red-500 text-white border-red-500" : "bg-white text-gray-500 border-gray-200 hover:border-red-300"
          }`}
        >
          Yes
        </button>
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-xs font-black border transition-all ${
            value === false ? "bg-gray-700 text-white border-gray-700" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"
          }`}
        >
          No
        </button>
      ...
      <svg width={130} height={165} viewBox="0 0 130 165" xmlns="http://www.w3.org/2000/svg">
        ...
        {boothType === "360 Video Booth" && (
          <>
            ...
            <text x={65} y={37} textAnchor="middle" fontSize={8} fontWeight="bold" fill="white">
              360° VIDEO
            </text>
          </>
        )}
        ...
        {boothType === "Open-Air Booth" && (
          <>
            ...
            {backdropStyle === "Plain Color" && <rect x={22} y={42} width={86} height={98} rx={2} fill={tc.secondary} />}
            ...
            {backdropStyle === "Custom Backdrop" && (
              <>
                <rect x={22} y={42} width={86} height={98} rx={2} fill={tc.secondary} />
                <text x={65} y={90} textAnchor="middle" fontSize={8} fill={tc.accent} fontWeight="bold">
                  CUSTOM
                </text>
                <text x={65} y={101} textAnchor="middle" fontSize={7} fill={tc.accent} opacity={0.6}>
                  BACKDROP
                </text>
              </>
            )}
        ...
        <text x={103} y={21} textAnchor="middle" fontSize={7.5} fontWeight="bold" fill="white">
          {duration}hrs
        </text>
      </svg>
...
export default function PortaBoothOrderForm() {
  ...
  const effectiveDuration =
    duration === "Custom" ? parseFloat(customDuration) || 2 : parseFloat(duration)

  const validate = () => {
    const e = {}
    if (eventType === "Others" && !otherEvent.trim()) e.otherEvent = "Please specify the event type"
    if (!eventDate) e.eventDate = "Please select an event date"
    else {
      const today = new Date()
      today.setHours(0, 0, 0, 0)
      if (new Date(eventDate) < today) e.eventDate = "Event date must be today or in the future"
    }
    if (duration === "Custom" && (!customDuration || parseFloat(customDuration) < 2)) e.customDuration = "Minimum 2 hours required"
    if (estimatedGuests < 1) e.guests = "Must have at least 1 guest"
    if (layoutMode === "upload" && !layoutFile) e.layout = "Please upload a layout file"
    if (layoutMode === "request" && !layoutEventName.trim()) e.layoutEventName = "Please enter the event name"
    if (!address.trim()) e.address = "Please enter the event address"
    setErrors(e)
  ...
  const BOOTH_TYPES = [
    { val: "Open-Air Booth", icon: <FaCampground />, desc: "Classic open setup — great for large events", badge: "Base" },
    { val: "Enclosed Booth", icon: <FaStore />, desc: "Private cabin — intimate, fully covered", badge: "Base" },
    { val: "360 Video Booth", icon: <FaCamera />, desc: "Rotating 360° video platform — ultra modern", badge: "+₱3,000" },
  ]
  const BACKDROP_STYLES = [
    { val: "Plain Color", icon: <FaPalette />, desc: "Solid-color vinyl — clean and modern" },
    { val: "Themed Backdrop", icon: <FaImage />, desc: "Pre-made themed prints — elegant patterns" },
    { val: "Custom Backdrop", icon: <FaWandMagicSparkles />, desc: "Your custom design or reference image", badge: "+₱1,500" },
  ]
  const BOOTH_THEMES = ["Elegant", "Fun & Colorful", "Minimalist", "Custom theme"]
  const PRINT_SIZES = ["2×6 Strip", "4×6 Photo", "Both"]
  const PRINT_COPIES = [
    { val: "1 Copy", icon: <FaRegFileLines />, desc: "One print per shot — standard" },
    { val: "2 Copies", icon: <FaCopy />, desc: "Two prints per shot — share with guests" },
    { val: "Unlimited Prints", icon: <FaInfinity />, desc: "Print as many as you like per session", badge: "+₱1,000" },
  ]
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 flex flex-col gap-6">
        <SectionCard title="Event Details" icon={<FaCalendarDays />}>
                ...
                {DURATION_OPTS.map((d) => (
                  <button
                    ...
                    className={`py-2.5 px-3 rounded-xl border-2 text-sm font-bold text-center transition-all ${
                      duration === d ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-500 hover:border-red-300"
                    }`}
                  >
        ...
        <SectionCard title="Booth Setup Options" icon={<FaCampground />}>
          <div className="flex flex-col gap-5">
            <Field label="Booth Type" required>
              <div className="flex flex-col gap-2">
                {BOOTH_TYPES.map(({ val, icon, desc, badge }) => (
                  <OptionCard key={val} active={boothType === val} onClick={() => setBoothType(val)} icon={icon} label={val} sublabel={desc} badge={badge} />
                ))}
              </div>
            </Field>
            ...
            <Field label="Backdrop Style" required>
              <div className="flex flex-col gap-2">
                {BACKDROP_STYLES.map(({ val, icon, desc, badge }) => (
                  <OptionCard
                    key={val}
                    active={backdropStyle === val}
                    onClick={() => setBackdropStyle(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                    badge={badge}
              ...
              {backdropStyle === "Custom Backdrop" && (
                <div className="mt-2">
                  ...
                  >
                    <span className="text-3xl group-hover:scale-110 transition-transform">{backdropFile ? <FaCircleCheck /> : <FaImage />}</span>
                    {backdropFile ? (
                    ...
                    <input ref={backdropRef} type="file" accept="image/*,.pdf" className="hidden" onChange={(e) => setBackdropFile(e.target.files[0] || null)} />
                  </div>
                </div>
              )}
            </Field>
              ...
              <div className="grid grid-cols-2 gap-2">
                {BOOTH_THEMES.map((t) => (
                  <button
                    key={t}
                    type="button"
                    onClick={() => setBoothTheme(t)}
                    className={`py-2.5 px-3 rounded-xl border-2 text-sm font-bold text-center transition-all ${
                      boothTheme === t ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-500 hover:border-red-300"
                    }`}
                  >
                    {t}
                  </button>
        ...
        <SectionCard title="Photo Print Options" icon={<FaPrint />}>
          <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {PRINT_SIZES.map((s) => (
                  <button
                    ...
                    className={`py-2.5 px-3 rounded-xl border-2 text-sm font-bold text-center transition-all ${
                      printSize === s ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-500 hover:border-red-300"
                    }`}
                  >
                    {s}
                  </button>
            ...
            <Field label="Print Copies per Shot" required>
              <div className="flex flex-col gap-2">
                {PRINT_COPIES.map(({ val, icon, desc, badge }) => (
                  <OptionCard key={val} active={printCopies === val} onClick={() => setPrintCopies(val)} icon={icon} label={val} sublabel={desc} badge={badge} />
                ))}
              </div>
            </Field>
            ...
            <Field label="Layout Design">
              <div className="flex gap-3 mb-3">
                <ToggleBtn active={layoutMode === "upload"} onClick={() => setLayoutMode("upload")}>
                  <FaFolderOpen className="inline-block mr-2 align-[-0.1em]" /> Upload Layout
                </ToggleBtn>
                <ToggleBtn active={layoutMode === "request"} onClick={() => setLayoutMode("request")}>
                  <FaPenToSquare className="inline-block mr-2 align-[-0.1em]" /> Request Design
                </ToggleBtn>
              </div>
              ...
              {layoutMode === "upload" && (
                <div className="flex flex-col gap-2">
                  ...
                  >
                    <span className="text-4xl group-hover:scale-110 transition-transform">{layoutFile ? <FaCircleCheck /> : <FaImage />}</span>
                    {layoutFile ? (
              ...
              {layoutMode === "request" && (
                <div className="flex flex-col gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  <Field label="Event Name" required>
                    <input
                      type="text"
                      value={layoutEventName}
                      onChange={(e) => setLayoutEventName(e.target.value)}
                  ...
                  <Field label="Theme Colors" hint="Describe or list hex codes">
                    <input
                      ...
                      onChange={(e) => setLayoutThemeColors(e.target.value)}
                  ...
                  <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700">
                    <FaTriangleExclamation />
                    <p>
                      Our design team will send you a preview within <strong>48 hours</strong> for approval before your event.
                    </p>
                  </div>
                </div>
              )}
        ...
        <SectionCard title="Personalization & Media" icon={<FaMobileScreenButton />}>
        ...
        <SectionCard title="Add-ons & Upgrades" icon={<FaStar />}>
          <div className="flex flex-col gap-3">
            <AddOnToggle
            ...
            <AddOnToggle
              ...
              priceLabel="+₱700"
              checked={redCarpet}
              onChange={() => setRedCarpet(!redCarpet)}
            />
            <AddOnToggle
              ...
              priceLabel="+₱800"
              checked={ledLighting}
              onChange={() => setLedLighting(!ledLighting)}
            />
            <AddOnToggle
              ...
              priceLabel="Included"
              checked={customFrame}
              onChange={() => setCustomFrame(!customFrame)}
            />
            <AddOnToggle
              ...
              icon={<FaUsers />}
              priceLabel="+₱1,000"
              checked={additionalStaff}
              onChange={() => setAdditionalStaff(!additionalStaff)}
            />
          </div>
        </SectionCard>

        <SectionCard title="Location & Setup" icon={<FaLocationDot />}>
          <div className="flex flex-col gap-5">
            <Field label="Event Location Type" required>
              <div className="flex gap-3">
                <ToggleBtn active={locationType === "Indoor"} onClick={() => setLocationType("Indoor")}>
                  <FaBuilding className="inline-block mr-2 align-[-0.1em]" /> Indoor
                </ToggleBtn>
                <ToggleBtn active={locationType === "Outdoor"} onClick={() => setLocationType("Outdoor")}>
                  <FaSun className="inline-block mr-2 align-[-0.1em]" /> Outdoor
                </ToggleBtn>
              </div>
              {locationType === "Outdoor" && (
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700 mt-2">
                  <FaTriangleExclamation />
                  <p>
                    For outdoor events, please ensure a <strong>covered or shaded area</strong> is available. Confirm power source access with our team.
                  </p>
                </div>
              )}
            </Field>
            ...
            <Field label="Setup Time Before Event" hint="How many hours early should we arrive?">
              <div className="flex items-center gap-3">
                <input type="number" min={0.5} step={0.5} value={setupTime} onChange={(e) => setSetupTime(e.target.value)} className={inputCls + " max-w-28"} />
                <span className="text-sm text-gray-500">hour{parseFloat(setupTime) !== 1 ? "s" : ""} before event</span>
              </div>
              <div className="flex items-start gap-3 bg-blue-50 border border-blue-100 rounded-xl p-3 text-xs text-blue-700 mt-2">
                <FaCircleInfo />
                <p>
                  We recommend at least <strong>1–2 hours</strong> for full setup, testing, and backdrop alignment.
                </p>
              </div>
            </Field>
          </div>
              ...
              {extraHrsCost > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-400">
                    Extra {effectiveDuration - 2} hr{effectiveDuration - 2 !== 1 ? "s" : ""} × ₱1,500
                  </span>
