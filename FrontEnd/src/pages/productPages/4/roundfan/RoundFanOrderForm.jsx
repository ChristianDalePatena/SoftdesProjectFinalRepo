  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap gap-3">
        ...
        <button type="button" title="Custom" onClick={() => onSelect("custom")} className="flex flex-col items-center gap-1 group">
          <div
            className={`w-10 h-10 rounded-full border-[3px] transition-all ${
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width="140" height="175" viewBox="0 0 140 175" xmlns="http://www.w3.org/2000/svg">
        ...
        {edgeStyle !== "Full Print (no border)" && <circle cx="70" cy="76" r={rimR} fill={rimHex} />}

        <circle cx="70" cy="76" r={faceR} fill="white" />
        <circle cx="70" cy="76" r={faceR - 10} fill={rimHex} opacity="0.07" />

        <text x="70" y="70" textAnchor="middle" fontSize="9" fontWeight="bold" fill={rimHex} opacity="0.65">
          YOUR
        </text>
        <text x="70" y="82" textAnchor="middle" fontSize="9" fontWeight="bold" fill={rimHex} opacity="0.65">
          DESIGN
        </text>
        {printStyle === "Double-sided" && (
          <text x="70" y="94" textAnchor="middle" fontSize="7" fill={rimHex} opacity="0.4">
            2-SIDED
          </text>
        )}

        <rect x="92" y="24" width="30" height="16" rx="8" fill={rimHex} opacity="0.9" />
        <text x="107" y="35" textAnchor="middle" fontSize="8" fontWeight="bold" fill="white">
          {sizeLabel}
        </text>
        ...
        {fanStyle === "Custom Shape Fan" && (
          <>
            ...
            <text x="70" y="175" textAnchor="middle" fontSize="6.5" fill="#9ca3af">
              Custom
            </text>
          </>
        )}
      ...
      <div className="flex items-center gap-2">
        <div className="w-3 h-3 rounded-full border border-gray-300 shrink-0" style={{ background: rimHex }} />
...
export default function RoundFanOrderForm() {
  ...
  const handleSubmit = () => {
    ...
    alert(
      `✅ Order submitted!\n\nFan: ${fanStyle}\nQty: ${quantity.toLocaleString()}\nBorder: ${
        BORDER_COLORS.find((c) => c.hex === borderColor)?.label ?? "Custom"
  ...
  const FAN_STYLES = [
    { val: "Round Fan (Standard)", icon: <FaFan />, desc: "Classic circle fan with back pocket for handle" },
    { val: "Round Fan with Handle Extension", icon: <FaFan />, desc: "Extended stick handle — better grip for campaigns & rallies" },
    { val: "Custom Shape Fan", icon: <FaScissors />, desc: "Die-cut to your preferred outline or shape", badge: "Custom" },
  ]

  const SIZES = ["6 inches", "8 inches", "10 inches", "Custom size"]
  const MATERIALS = [
    { val: "Cardboard", icon: <FaRegFileLines />, desc: "Lightweight, eco-friendly — great for high-volume events" },
    { val: "PVC Board", icon: <FaShieldHalved />, desc: "Durable rigid board — weather-resistant", badge: "+₱5/pc" },
    { val: "Sintra Board", icon: <FaBox />, desc: "Foam PVC — lightweight yet sturdy and stiff", badge: "+₱5/pc" },
  ]
  const PRINT_SURFACES = [
    { val: "Direct Print", icon: <FaPrint />, desc: "Ink printed directly onto the fan surface" },
    { val: "Sticker Print", icon: <FaImage />, desc: "Full-face sticker applied over the fan board" },
  ]
  const HANDLE_TYPES = [
    { val: "Plastic Handle", icon: <FaCircle />, desc: "Standard white flat plastic stick" },
    { val: "Die-cut Handle", icon: <FaScissors />, desc: "Handle integrated as part of the fan cutout" },
    { val: "No Handle", icon: <FaCircleInfo />, desc: "No handle — mount, glue, or use as is" },
  ]
  ...
  const summaryRows = [
    ...
    { label: "Design", value: designMode === "upload" ? "File Upload" : "Manual Input" },
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 flex flex-col gap-6">
        <SectionCard title="Fan Details" icon={<FaFan />}>
            ...
            <Field label="Fan Size" required>
                ...
                {SIZES.map((s) => (
                  <button
                    ...
                    className={`py-2.5 px-3 rounded-xl border-2 text-sm font-bold text-center transition-all ${
                      size === s ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-500 hover:border-red-300"
                    }`}
                  >
                    {s}
                  </button>
              ...
              {(size === "8 inches" || size === "10 inches") && (
                <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-2 text-xs text-amber-700 mt-1">
                  <FaTriangleExclamation />
                  <span>
                    This size adds <strong>₱3/pc</strong> to the base price.
                  </span>
                </div>
              )}
            </Field>
        ...
        <SectionCard title="Material & Build" icon={<FaBox />}>
          <div className="flex flex-col gap-5">
            ...
            <Field label="Print Surface" required>
              <div className="flex flex-col gap-2">
                {PRINT_SURFACES.map(({ val, icon, desc }) => (
                  <OptionCard
                    key={val}
                    active={printSurface === val}
                    onClick={() => setPrintSurface(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                  />
            ...
            <Field label="Handle Type" required>
              <div className="flex flex-col gap-2">
                {HANDLE_TYPES.map(({ val, icon, desc }) => (
                  <OptionCard
                    key={val}
                    active={handleType === val}
                    onClick={() => setHandleType(val)}
                    icon={icon}
                    label={val}
                    sublabel={desc}
                  />
        ...
        <SectionCard title="Printing Options" icon={<FaPrint />}>
          <div className="flex flex-col gap-5">
            <Field label="Print Style" required>
              <div className="flex gap-3">
                {["Single-sided", "Double-sided"].map((s) => (
                  <ToggleBtn key={s} active={printStyle === s} onClick={() => setPrintStyle(s)}>
                    {s === "Single-sided" ? (
                      <>
                        <FaSquare className="inline-block mr-2 align-[-0.1em]" /> Single-sided
                      </>
                    ) : (
                      <>
                        <FaClone className="inline-block mr-2 align-[-0.1em]" /> Double-sided
                      </>
                    )}
                  </ToggleBtn>
                ))}
              ...
              {printStyle === "Double-sided" && (
                <div className="flex items-center gap-2 bg-blue-50 border border-blue-100 rounded-xl px-4 py-2 text-xs text-blue-700 mt-2">
                  <FaCircleInfo />
                  <span>
                    Adds <strong>₱3/pc</strong>. Provide a second design file for the back face, or we'll mirror the front.
                  </span>
                </div>
              )}
            </Field>
                ...
                {["Standard Colored Rim", "Full Print (no border)"].map((s) => (
                  <button
                    ...
                    className={`flex-1 py-3 px-3 rounded-xl border-2 text-sm font-bold text-center transition-all ${
                      edgeStyle === s ? "border-red-500 bg-red-50 text-red-700" : "border-gray-200 bg-white text-gray-500 hover:border-red-300"
                    }`}
                  >
                    {s === "Standard Colored Rim" ? (
                      <>
                        <FaCircle className="inline-block mr-2 align-[-0.1em]" /> Colored Rim
                      </>
                    ) : (
                      <>
                        <FaImage className="inline-block mr-2 align-[-0.1em]" /> Full Print
                      </>
                    )}
                  </button>
                ))}
        ...
        <SectionCard title="Design & Customization" icon={<FaPenToSquare />}>
          <div className="flex flex-col gap-5">
            <Field label="Design Method" required>
              <div className="flex gap-3">
                <ToggleBtn active={designMode === "upload"} onClick={() => setDesignMode("upload")}>
                  <FaFolderOpen className="inline-block mr-2 align-[-0.1em]" /> Upload Design
                </ToggleBtn>
                <ToggleBtn active={designMode === "manual"} onClick={() => setDesignMode("manual")}>
                  <FaPenToSquare className="inline-block mr-2 align-[-0.1em]" /> Manual Input
                </ToggleBtn>
              </div>
            </Field>
              ...
              <div className="flex flex-col gap-3">
                ...
                >
                  <span className="text-4xl group-hover:scale-110 transition-transform">
                    {designFile ? <FaCircleCheck /> : <FaPalette />}
                  </span>
                  {designFile ? (
                ...
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700">
                  <FaTriangleExclamation />
                  <p>
                    Use the correct fan circle dimensions. Add a 3mm bleed outside the trim line. The rim color area will be covered by the border.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex flex-col gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                ...
                <Field label="Color Theme" hint="Main design color — ideally matches the rim">
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <input
                      type="text"
                      value={colorThemeDesc}
                      onChange={(e) => setColorThemeDesc(e.target.value)}
                      placeholder="e.g. Red & white, Blue & gold"
                      className={inputCls}
                    />
                  </div>
                </Field>
                ...
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700">
                  <FaTriangleExclamation />
                  <p>
                    Our design team will send a digital proof within <strong>24–48 hours</strong> for your approval before mass printing begins.
                  </p>
                </div>
              </div>
            )}
            ...
            <Field label="Photo Upload" hint="Candidate photo, event host, or group photo — very common for campaign fans">
              ...
              >
                <span className="text-2xl group-hover:scale-110 transition-transform shrink-0">
                  {photoFile ? <FaCircleCheck /> : <FaCamera />}
                </span>
                {photoFile ? (
                  <div className="flex-1 min-w-0">
        ...
        <SectionCard title="Bulk Personalization" icon={<FaUsers />}>
          <div className="flex flex-col gap-5">
            <div className="flex items-center gap-4 p-4 rounded-xl border-2 border-gray-200 bg-white">
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0 ${bulkOrder ? "bg-red-500" : "bg-gray-300"}`}
              >
                <span
                  className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${bulkOrder ? "translate-x-6" : "translate-x-1"}`}
                />
              </button>
            </div>
            ...
            {bulkOrder && (
              <div className="flex flex-col gap-3">
                ...
                >
                  <span className="text-4xl group-hover:scale-110 transition-transform">{csvFile ? <FaCircleCheck /> : <FaFileCsv />}</span>
                  {csvFile ? (
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-xs text-blue-700">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {["Full Name", "Position / Title", "Photo Filename"].map((col) => (
                      <div key={col} className="bg-blue-100 rounded-lg px-3 py-1.5 text-center font-semibold text-blue-800">
                        {col}
                      </div>
                    ))}
                  </div>
                  <p className="mt-2">
                    Each row = one fan design. Total rows must match your quantity. Include all photos in a ZIP file together with the spreadsheet.
                  </p>
                </div>
              </div>
            )}
        ...
        <SectionCard title="Add-ons & Finishing" icon={<FaStar />}>
          <div className="flex flex-col gap-3">
            <AddOnToggle
              label="Double-Sided Print"
              sublabel="Print on both front and back face of the fan"
              icon={<FaClone />}
              priceLabel="+₱3/pc"
              checked={doubleSided}
            ...
            <AddOnToggle
              ...
              sublabel="Shiny protective film — vivid colors and glossy feel"
              icon={<FaWandMagicSparkles />}
              priceLabel="+₱2/pc"
              checked={glossLam}
              onChange={handleGloss}
            />
            <AddOnToggle
              label="Matte Lamination"
              sublabel="Smooth non-reflective finish — clean and premium"
              icon={<FaShieldHalved />}
              priceLabel="+₱2/pc"
              checked={matteLam}
              onChange={handleMatte}
            />
            {(glossLam || matteLam) && (
            ...
            <AddOnToggle
              label="UV Coating"
              sublabel="Protective UV layer — extends print life outdoors and in the sun"
              icon={<FaSun />}
              priceLabel="+₱2/pc"
              checked={uvCoating}
              onChange={() => setUvCoating(!uvCoating)}
            />
            <AddOnToggle
              label="Individual Packaging"
              sublabel="Each fan wrapped in a clear OPP bag — perfect for distribution"
              icon={<FaBagShopping />}
              priceLabel="+₱2/pc"
              checked={packaging}
        ...
        <SectionCard title="Delivery & Pickup" icon={<FaLocationDot />}>
          <div className="flex flex-col gap-5">
            <Field label="Delivery Method" required>
              <div className="flex gap-3">
                <ToggleBtn active={delivery === "Pickup"} onClick={() => setDelivery("Pickup")}>
                  <FaStore className="inline-block mr-2 align-[-0.1em]" /> Pickup
                </ToggleBtn>
                <ToggleBtn active={delivery === "Delivery"} onClick={() => setDelivery("Delivery")}>
                  <FaTruck className="inline-block mr-2 align-[-0.1em]" /> Delivery
                </ToggleBtn>
              </div>
            </Field>
      ...
      <div className="xl:col-span-1">
        <div className="sticky top-[8.75rem] flex flex-col gap-4">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
            ...
            <div className="px-6 py-6 flex flex-col items-center gap-3 bg-gray-50 min-h-56">
              <FanPreview rimHex={rimHex} size={size} fanStyle={fanStyle} edgeStyle={edgeStyle} printStyle={printStyle} />
          ...
          <div className="bg-yellow-50 border border-yellow-100 rounded-2xl p-5">
            ...
            <p className="text-xs text-yellow-700 leading-relaxed">
              ...
              for bulk quotes, rush orders, and design assistance.
            </p>
          </div>
        </div>
      </div>
