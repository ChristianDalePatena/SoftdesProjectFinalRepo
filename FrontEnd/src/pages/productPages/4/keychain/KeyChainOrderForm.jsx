  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width={120} height={150} viewBox="0 0 100 130" xmlns="http://www.w3.org/2000/svg">
        {attachmentType === "Standard Keyring" && <ellipse cx={50} cy={10} rx={10} ry={6} fill="none" stroke="#9ca3af" strokeWidth={2.5} />}
        ...
        {attachmentType === "Chain + Ring" && (
          <>
            ...
            {[0, 1, 2].map((i) => (
              <ellipse key={i} cx={50} cy={15 + i * 4} rx={3} ry={2} fill="none" stroke="#9ca3af" strokeWidth={1.2} />
            ))}
          </>
        )}
        ...
        <line
          x1={50}
          y1={attachmentType === "Chain + Ring" ? 27 : 16}
          x2={50}
          y2={22}
          stroke="#9ca3af"
          strokeWidth={1.2}
        ...
        {s.type === "circle" ? (
          ...
          <rect x={s.x} y={s.y} width={s.w} height={s.h} rx={s.rx} fill={tc.fill} stroke={tc.stroke} strokeWidth={1.5} />
        )}

        {holographic &&
          holoColors.map((c, i) =>
            s.type === "circle" ? (
              <circle key={i} cx={s.cx} cy={s.cy} r={s.r - i * 5} fill="none" stroke={c} strokeWidth={2} opacity={0.18} />
            ) : (
              <rect
                key={i}
                x={s.x + i * 4}
        ...
        {tc.shine &&
          finishOpacity > 0 &&
          (s.type === "circle" ? (
            <ellipse cx={s.cx - 10} cy={s.cy - 12} rx={12} ry={8} fill="white" opacity={finishOpacity} />
          ) : (
            <ellipse cx={s.x + 16} cy={s.y + 12} rx={14} ry={8} fill="white" opacity={finishOpacity} />
          ))}
        ...
        {doubleSided && (
          <g>
            ...
            <text x={83} y={108} textAnchor="middle" fontSize={6} fontWeight="bold" fill="white">
              2-SD
            </text>
          </g>
        )}
...
export default function KeyChainOrderForm() {
  ...
  const KEYCHAIN_TYPES = [
    { val: "Acrylic Keychain", icon: Sparkles, desc: "Clear, vibrant colors — most popular", badge: "+₱10" },
    { val: "PVC Rubber Keychain", icon: CircleDashed, desc: "Soft, flexible, 3D-embossed designs", badge: "+₱15" },
    { val: "Metal Keychain", icon: Cog, desc: "Premium, engraved, durable", badge: "+₱20" },
    { val: "Wooden Keychain", icon: TreePine, desc: "Natural, laser-engraved aesthetic", badge: "+₱12" },
    { val: "Epoxy Keychain", icon: Gem, desc: "Glossy dome coat — ultra-vibrant", badge: "+₱15" },
  ]
  ...
  const FINISHES = [
    { val: "Glossy", icon: Sparkles, desc: "Shiny, vibrant, reflective surface" },
    { val: "Matte", icon: Layers3, desc: "Soft non-glare, smooth feel" },
    { val: "Epoxy Coated", icon: Gem, desc: "Thick dome resin — premium protection", badge: "+₱5" },
  ]
  const ATTACHMENTS = [
    { val: "Standard Keyring", icon: Circle, desc: "Classic split ring — universal fit" },
    { val: "Lobster Clasp", icon: Link2, desc: "Secure snap hook — won't fall off" },
    { val: "Chain + Ring", icon: Link, desc: "Stylish chain link with ring" },
  ]
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <div className="xl:col-span-2 flex flex-col gap-6">
        <SectionCard title="Keychain Details" icon={KeyRound}>
          <div className="flex flex-col gap-5">
            <Field label="Keychain Type" required>
              <div className="flex flex-col gap-2">
                {KEYCHAIN_TYPES.map(({ val, icon, desc, badge }) => (
                  <OptionCard key={val} active={keychainType === val} onClick={() => setKeychainType(val)} icon={icon} label={val} sublabel={desc} badge={badge} />
                ))}
              </div>
            </Field>
        ...
        <SectionCard title="Material & Finish" icon={Box}>
          <div className="flex flex-col gap-5">
            <div className="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl px-4 py-3">
              <WandSparkles className="h-5 w-5 text-gray-500" />
              <div>
            ...
            <Field label="Finish">
              <div className="flex flex-col gap-2">
                {FINISHES.map(({ val, icon, desc, badge }) => (
                  <OptionCard key={val} active={finish === val} onClick={() => setFinish(val)} icon={icon} label={val} sublabel={desc} badge={badge} />
                ))}
              </div>
            </Field>
            ...
            <Field label="Printing Style">
              <div className="flex gap-3">
                <ToggleBtn active={printingStyle === "Single-sided"} onClick={() => setPrintingStyle("Single-sided")}>
                  <Square className="inline-block h-4 w-4 mr-1.5" />
                  Single-sided
                </ToggleBtn>
                <ToggleBtn active={printingStyle === "Double-sided"} onClick={() => setPrintingStyle("Double-sided")}>
                  <Layers3 className="inline-block h-4 w-4 mr-1.5" />
                  Double-sided
        ...
        <SectionCard title="Customization" icon={Paintbrush}>
          <div className="flex flex-col gap-5">
            <Field label="Design Mode">
              <div className="flex gap-3">
                ...
                <ToggleBtn active={designMode === "text"} onClick={() => setDesignMode("text")}>
                  <PenLine className="inline-block h-4 w-4 mr-1.5" />
                  Text-Based
                </ToggleBtn>
              </div>
            </Field>
            ...
            {designMode === "upload" && (
              <Field label="Upload Design File" hint="PNG, JPG, PDF, AI accepted · 300 DPI recommended · Transparent background preferred" required>
                <div
                ...
                >
                  {designFile ? <CheckCircle2 className="h-10 w-10 text-green-600 group-hover:scale-110 transition-transform" /> : <ImageUp className="h-10 w-10 text-gray-400 group-hover:scale-110 transition-transform" />}
                  {designFile ? (
                ...
                <div className="flex items-start gap-3 bg-amber-50 border border-amber-100 rounded-xl p-3 text-xs text-amber-700 mt-2">
                  ...
                  <p>
                    For small keychains, keep details simple. Thin lines below <strong>0.5mm</strong> may not print clearly. Transparent PNG gives the best cut results.
                  </p>
                </div>
              </Field>
            )}
              ...
              <div className="flex flex-col gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                ...
                <Field label="Subtext" hint="Tagline, date, or secondary info">
                  <input
                    ...
                    placeholder="e.g. Batch 2025, BFF, Est. 2001"
                    className={inputCls}
                  />
                </Field>
            ...
            <Field label="Attachment Type">
              <div className="flex flex-col gap-2">
                {ATTACHMENTS.map(({ val, icon, desc }) => (
                  <OptionCard key={val} active={attachmentType === val} onClick={() => setAttachmentType(val)} icon={icon} label={val} sublabel={desc} />
                ))}
              </div>
            </Field>
          </div>
        ...
        <SectionCard title="Bulk Order" icon={Package}>
          <div className="flex flex-col gap-4">
            ...
            {bulkOrder && (
              <div className="flex flex-col gap-3 bg-blue-50 border border-blue-100 rounded-xl p-4">
                ...
                >
                  {csvFile ? <CheckCircle2 className="h-9 w-9 text-green-600 group-hover:scale-110 transition-transform" /> : <FileSpreadsheet className="h-9 w-9 text-blue-500 group-hover:scale-110 transition-transform" />}
                  {csvFile ? (
                ...
                <a href="#" className="text-[11px] text-blue-600 underline font-semibold">
                  <FileDown className="inline-block h-3.5 w-3.5 mr-1 align-[-2px]" />
        ...
        <SectionCard title="Add-ons & Effects" icon={Sparkles}>
          <div className="flex flex-col gap-3">
            ...
            <AddOnToggle
              ...
              priceLabel="+₱4"
              checked={holographic}
              onChange={() => setHolographic(!holographic)}
            />
            <AddOnToggle
              ...
              icon={ShieldCheck}
              priceLabel="+₱2"
              checked={protectiveFilm}
              onChange={() => setProtectiveFilm(!protectiveFilm)}
            />
            <AddOnToggle
